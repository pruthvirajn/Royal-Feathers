...
public class PlasmaEmitter02Level02 : MonoBehaviour {
	...
	IEnumerator moveIt(){
		...
		storeCageState02 = cage02.GetComponent<CageRotate02> ().cageState;
		switch (nextPlasma [plasmaCount02 + 1]) {
		...
		if (!coroutineRunnning) {
			if (plasmaCount02 < totalPlasmaCount02 && level.GetComponent<FormulaBar02>().islevelComplete == false) {
